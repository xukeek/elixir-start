defmodule NotionReaderWeb.Endpoint do
  use Sentry.PlugCapture
  use Phoenix.Endpoint, otp_app: :notion_reader

  alias Plug.Conn

  @plug_ssl Plug.SSL.init(rewrite_on: [:x_forwarded_proto])

  socket("/socket", NotionReaderWeb.Socket)
  ...
  if code_reloading? do
    ...
    plug(Phoenix.Ecto.CheckRepoStatus, otp_app: :notion_reader)
  end
  ...
  plug(NotionReaderHealth.Router)
  plug(NotionReaderGraphQL.Router)
  plug(:halt_if_sent)
  plug(NotionReaderWeb.Router)
  ...
  def init(_key, config) do
    if config[:load_from_system_env] do
      port = Application.get_env(:notion_reader, __MODULE__)[:http][:port] || raise "expected the PORT environment variable to be set"
  ...
  defp ping(%{request_path: "/ping"} = conn, _opts) do
    version = Application.get_env(:notion_reader, :version)
  ...
  defp canonical_host(conn, _opts) do
    opts = PlugCanonicalHost.init(canonical_host: Application.get_env(:notion_reader, :canonical_host))
  ...
  defp force_ssl(conn, _opts) do
    if Application.get_env(:notion_reader, :force_ssl) do
  ...
  defp cors(conn, _opts) do
    opts = Corsica.init(Application.get_env(:notion_reader, Corsica))

    Corsica.call(conn, opts)
  end

  defp basic_auth(conn, _opts) do
    basic_auth_config = Application.get_env(:notion_reader, :basic_auth)
